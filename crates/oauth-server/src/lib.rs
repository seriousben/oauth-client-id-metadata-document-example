...
use oauth_metadata::{
    hello_oauth, JsonWebKeySet, JwtIssuer, KeyManager, OAuthClientMetadata,
};
...
pub async fn token_endpoint(State(state): State<AppState>) -> (StatusCode, Json<Value>) {
    ...
    match state.jwt_issuer.create_client_credentials_response(Some("read write".to_string())) {
        Ok(response) => (
            StatusCode::OK,
...
pub async fn jwt_endpoint(
    ...
    match state.jwt_issuer.create_client_credentials_response_with_client_id(scope, params.client_id) {
        Ok(response) => (
            StatusCode::OK,
            Json(serde_json::to_value(response).unwrap_or_default()),
        ),
